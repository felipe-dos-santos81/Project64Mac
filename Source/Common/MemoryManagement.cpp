...
void * CommitMemory(void * addr, size_t size, MEM_PROTECTION memProtection)
{
    ...
    // Apple Silicon has no way to get RWX pages at a caller-supplied (MAP_FIXED)
    // address: MAP_JIT only ever hands back a kernel-chosen address, and a plain
    // PROT_EXEC mapping with MAP_FIXED is rejected (EACCES) without ad-hoc
    // codesigning plus the com.apple.security.cs.allow-jit entitlement, which this
    // interpreter-only build does not have. CommitMemory is only ever asked to
    // commit pages inside a region ReserveMemory already carved out at a fixed
    // address, so we drop PROT_EXEC here and map read/write only. This is fine as
    // long as the recompiler stays disabled on this port; if someone turns the x64
    // recompiler back on for Apple Silicon, this is the place that needs a real
    // W^X (MAP_JIT + pthread_jit_write_protect_np) redesign.
    OsMemProtection &= ~PROT_EXEC;
    void * ptr = mmap(addr, size, OsMemProtection, MAP_FIXED | MAP_PRIVATE | MAP_ANON, -1, 0);
