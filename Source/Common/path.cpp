...
void CPath::Module(void * /*hInstance*/)
{
    // hInstance (an HMODULE on Windows) has no POSIX equivalent - there is only
    // ever one running image - so it is accepted for signature compatibility
    // and ignored. _NSGetExecutablePath gives the path used to launch the
    // process (which may contain symlinks or be relative); realpath() resolves
    // it to the canonical absolute path, matching what GetModuleFileName
    // returns on Windows. This intentionally resolves inside Bin/macOS/ (not
    // an app-bundle Resources path); later tasks that build a .app bundle can
    // revisit this if they need bundle-relative resolution instead.
    char buff_path[PATH_MAX];
    memset(buff_path, 0, sizeof(buff_path));
    uint32_t size = sizeof(buff_path);
    if (_NSGetExecutablePath(buff_path, &size) != 0)
    {
        // buffer too small (size now holds the required size); give up rather
        // than truncate a path we would silently use as a base directory.
        return;
    }
    char resolved_path[PATH_MAX];
    if (realpath(buff_path, resolved_path) != nullptr)
    {
        m_strPath = resolved_path;
    }
    else
    {
        m_strPath = buff_path;
