...
uint32_t CThread::GetCurrentThreadId(void)
{
    ...
    // macOS defines SYS_gettid but not __NR_gettid, and there is no gettid()
    // libc wrapper either; pthread_threadid_np is the portable way to get a
    // per-thread id here. It returns a 64-bit id, so narrow to the uint32_t
