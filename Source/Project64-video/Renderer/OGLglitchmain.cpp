...
bool gfxSstWinOpen(gfxColorFormat_t color_format, gfxOriginLocation_t origin_location, int nColBuffers, int nAuxBuffers)
{
    ...
    // The frontend created the window and the GL context and made it current on this
    // thread before starting emulation, so there is no context to open here.
    WriteTrace(TraceGlitch, TraceDebug, "Using the GL context supplied by the frontend");
#else
...
#else // _WIN32
    // The frontend owns the GL context on every non-Windows platform, so presenting
    // a frame means asking it to swap. GFX_INFO::SwapBuffers is supplied by the core
    // (CGfxPlugin::SwapBuffers), which forwards to RenderWindow::SwapWindow.
